...
def neighbor_check(arr, x, y):
    a = 0
    # Checks Upper Left Neighboring cell
    if arr[x - 1, y - 1] is not None and arr[x - 1, y - 1] == 2:
        a += 1

    # Checks Top Neighboring cell
    if arr[x - 1, y] is not None and arr[x - 1, y] == 2:
        a += 1
    ...
    if arr[x, y - 1] is not None and arr[x, y - 1] == 2:
        a += 1
